package com.high.highblog.constant;

public class AppErrorCode {
    public static final String INVALID_JWT_TOKEN = "HB-0101";
    public static final String EXPIRED_JWT_TOKEN = "HB-0102";
    public static final String UNSUPPORTED_JWT_TOKEN = "HB-0103";
    public static final String EMPTY_CLAIMS = "HB-0104";
    public static final String INVALID_LOGIN_INFO = "HB-0105";
    public static final String DEFAULT_VALIDATOR = "HB-0100";
    public static final String INVALID_EMAIL_INFO = "HB-0106";
    public static final String DEFAULT_OBJECT_NOT_FOUND = "HB-0600";
}
